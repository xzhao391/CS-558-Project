...
if __name__ == "__main__":
    ...
    # Get dataset
    conf_dataset = [[-0.610788, -0.504067, 0.023066], [0.512449, -0.386098, -1.222586]]
    ...
    CAE = CAE_3d
    MLP = model.MLP
    mpNet = End2EndMPNet(total_input_size, AE_input_size, mlp_input_size, output_size, CAE, MLP)
    ...
    for _ in range(50):
        time0 = time.time()
        conf_dataset[0] = [-0.6+np.random.uniform(-1, 1), -0.5+np.random.uniform(-1, 1), -0.75+np.random.uniform(-1, 1)]
        conf_dataset[1] = [.7+np.random.uniform(-1, 1), -0.5+np.random.uniform(-1, 1), -0.75+np.random.uniform(-1, 1)]
        rgb_dataset = []
        ...
        for t in range(MAX_NEURAL_REPLAN):
            path = neural_plan(mpNet, path, obc[0], obs[0], IsInCollision, \
                                normalize_func, unnormalize_func, t==0, step_sz=step_sz)
            path = lvc(path, obc[0], IsInCollision, step_sz=step_sz)
            if feasibility_check(path, obc[0], IsInCollision, step_sz=step_sz):
                found_path = True
                n_successful_cur += 1
                break
    ...
    avg_time = sum_time/ 50
    success_rate = count/ 50
    print(success_rate, avg_time)